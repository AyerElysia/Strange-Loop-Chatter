...
def _format_inherent_block(records: list[Any]) -> str:
    ...
    return (
        "## 固有记忆\n"
        "以下内容来自长期记忆层，属于当前已生效的全局背景：\n"
        f"{body}\n"
        "（只在长期背景明确变化时才整体改写，不要把短期上下文当成固有记忆。）"
    )
