...
class BaseChatter(ABC):
    ...
    @abstractmethod
    async def execute(self, chat_stream: ChatStream) -> tuple[Annotated[bool, "是否成功"], Annotated[str, "结果详情"]]:
        """
        执行聊天处理逻辑
...
使用示例：
```python
from src.app.plugin_system import BaseChatter
