...
logger = get_logger("default_chatter_debug")
...
def log_dc_result(response: Any, config: DefaultChatterConfig) -> None:
    """美化输出 LLM 响应摘要（适配 DefaultChatter 提取 Call）。
    ...
    if not hasattr(response, "call_list") or not response.call_list:
        if hasattr(response, "message") and response.message:
            logger.info(f"[bold magenta]📝[/bold magenta] 纯文本回复:\n{response.message[:200]}...")
        return
    ...
    for call in response.call_list:
        ...
        if call_name == "tool-think":
            thought = args.get("thought", "")
            if thought:
                logger.info(f"[bold magenta]💭[/bold magenta] {thought}")
            mood = args.get("mood", "")
            if mood:
                logger.info(f"[dim]心情: {mood}[/dim]")
        elif call_name.startswith("action-send_text") or call_name == "send_text":
