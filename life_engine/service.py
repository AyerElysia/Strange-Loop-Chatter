...
class LifeEngineService(BaseService):
    ...
    def _build_heartbeat_model_prompt(self, wake_context: str) -> str:
        ...
        lines.extend([
            ...
            "### 你现在可以做什么",
            "",
            "1. **查看待办** - 用 `nucleus_list_todos` 看看有什么想做的事（⚠️ 必做）",
            "   - 注意 **overdue_count**（逾期）和 **urgent_count**（3天内截止）",
            "   - 逾期的 TODO 要么立刻做，要么删掉，不要拖延",
            ...
            "### 执行约定",
            "",
            "- **先看待办**：每次心跳必须先 `nucleus_list_todos` 检查待办状态",
            "- **TODO 是要做的**：创建的待办不是为了记录，是要去完成的。有截止时间的更要重视！",
            "- **截止时间压力**：",
            "  - 已逾期：立刻采取行动或删除",
            "  - 3天内：优先处理",
            "  - 7天内：开始规划",
